...
CHARTS_BASE_URL="${NEXT_PUBLIC_ATLAS_CHARTS_BASE_URL:-}"
CHART_HISTOGRAM_ID="${NEXT_PUBLIC_CHART_PRICE_HISTOGRAM_ID:-}"
CHART_PAYER_ID="${NEXT_PUBLIC_CHART_PAYER_BREAKDOWN_ID:-}"
CHART_TREND_ID="${NEXT_PUBLIC_CHART_PRICE_TREND_ID:-}"
DASHBOARD_ID="${NEXT_PUBLIC_ATLAS_DASHBOARD_ID:-}"
...
gcloud run deploy clearprice-mcp \
  ...
  --set-env-vars "MONGODB_URI=${MONGO_URI},MONGODB_DATABASE=clearprice,NODE_ENV=production,GCP_PROJECT_ID=${PROJECT},VERTEX_AI_LOCATION=${REGION},GOOGLE_MAPS_API_KEY=${MAPS_KEY},GEMINI_API_KEY=${GEMINI_KEY}" \
  --memory 512Mi \
  --min-instances 1
...
ENV_VARS="NEXT_PUBLIC_API_URL=${API_URL},NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=${MAPS_KEY}"
BUILD_ENV_VARS="NEXT_PUBLIC_API_URL=${API_URL},NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=${MAPS_KEY}"

if [ -n "$CHARTS_BASE_URL" ]; then
  echo "NEXT_PUBLIC_ATLAS_CHARTS_BASE_URL=${CHARTS_BASE_URL}" >> frontend/.env.production
  ENV_VARS="${ENV_VARS},NEXT_PUBLIC_ATLAS_CHARTS_BASE_URL=${CHARTS_BASE_URL}"
  BUILD_ENV_VARS="${BUILD_ENV_VARS},NEXT_PUBLIC_ATLAS_CHARTS_BASE_URL=${CHARTS_BASE_URL}"
fi
if [ -n "$CHART_HISTOGRAM_ID" ]; then
  echo "NEXT_PUBLIC_CHART_PRICE_HISTOGRAM_ID=${CHART_HISTOGRAM_ID}" >> frontend/.env.production
  ENV_VARS="${ENV_VARS},NEXT_PUBLIC_CHART_PRICE_HISTOGRAM_ID=${CHART_HISTOGRAM_ID}"
  BUILD_ENV_VARS="${BUILD_ENV_VARS},NEXT_PUBLIC_CHART_PRICE_HISTOGRAM_ID=${CHART_HISTOGRAM_ID}"
fi
if [ -n "$CHART_PAYER_ID" ]; then
  echo "NEXT_PUBLIC_CHART_PAYER_BREAKDOWN_ID=${CHART_PAYER_ID}" >> frontend/.env.production
  ENV_VARS="${ENV_VARS},NEXT_PUBLIC_CHART_PAYER_BREAKDOWN_ID=${CHART_PAYER_ID}"
  BUILD_ENV_VARS="${BUILD_ENV_VARS},NEXT_PUBLIC_CHART_PAYER_BREAKDOWN_ID=${CHART_PAYER_ID}"
fi
if [ -n "$CHART_TREND_ID" ]; then
  echo "NEXT_PUBLIC_CHART_PRICE_TREND_ID=${CHART_TREND_ID}" >> frontend/.env.production
  ENV_VARS="${ENV_VARS},NEXT_PUBLIC_CHART_PRICE_TREND_ID=${CHART_TREND_ID}"
  BUILD_ENV_VARS="${BUILD_ENV_VARS},NEXT_PUBLIC_CHART_PRICE_TREND_ID=${CHART_TREND_ID}"
fi
if [ -n "$DASHBOARD_ID" ]; then
  echo "NEXT_PUBLIC_ATLAS_DASHBOARD_ID=${DASHBOARD_ID}" >> frontend/.env.production
  ENV_VARS="${ENV_VARS},NEXT_PUBLIC_ATLAS_DASHBOARD_ID=${DASHBOARD_ID}"
  BUILD_ENV_VARS="${BUILD_ENV_VARS},NEXT_PUBLIC_ATLAS_DASHBOARD_ID=${DASHBOARD_ID}"
fi
